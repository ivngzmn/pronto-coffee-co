...
export function CustomerAuthExperience({ apiUrl }) {
  ...
  async function submit(event) {
    ...
    const path = mode === "signup" ? "/api/customer/auth/signup" : "/api/customer/auth/login";
  ...
  return (
    <div className="grid min-h-[calc(100vh-4rem)] items-center gap-10 px-4 py-16 md:px-6 lg:grid-cols-[0.95fr_1.05fr]">
      <section className="hidden space-y-6 border-r border-border pr-10 lg:block">
        ...
        <p className="max-w-md text-base leading-8 text-muted-foreground">
          Create a Pronto customer account to send pickup orders into the same live queue our baristas use at the counter.
        </p>
        <div className="grid gap-4">
          {[
            ["Instant Pickup", "Build your order before you arrive and keep your morning moving."],
            ["Protected Queue", "Signed-in orders help the team keep spam out of the barista workflow."],
      ...
      <Card className="mx-auto w-full max-w-md">
        <CardHeader>
          <CardTitle className="text-3xl">{mode === "signup" ? "Create an account" : "Welcome back"}</CardTitle>
          <CardDescription>
            {mode === "signup"
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={submit}>
            {mode === "signup" ? (
              <Field label="Name">
                <Input name="userName" required placeholder="Taylor" autoComplete="name" />
              </Field>
            ) : null}
            <Field label="Email">
            ...
            <Button className="w-full" type="submit" disabled={pending || session.loading}>
              {pending ? "Working..." : mode === "signup" ? "Create account" : "Login"}
            </Button>
          </form>
